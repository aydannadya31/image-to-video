...
class AgnesBridge {
  ...
  async generate(jobId, imagePath, callbacks = {}) {
    ...
    try {
      ...
      if (!videoId) {
        throw new Error('Agnes: video ID alınamadı: ' + JSON.stringify(resp.data));
      }

      const result = await this._pollVideo(videoId, onProgress);
      if (!result || !result.video_url) {
        throw new Error('Agnes: video URL alınamadı');
      }
  ...
  async _pollVideo(videoId, onProgress) {
    ...
    for (let i = 0; i < this.maxPolls; i++) {
      ...
      try {
        ...
        if (status === 'failed') {
          throw new Error('Agnes: video üretimi başarısız');
        }
    ...
    throw new Error('Agnes: zaman aşımı');
  }
